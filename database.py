...
class Database:
    ...
    def add_referral(self, inviter_id, invited_id):
        """Do'stini taklif qilish"""
        try:
            self.conn.execute(
                "CREATE TABLE IF NOT EXISTS referrals (id INTEGER PRIMARY KEY AUTOINCREMENT, inviter_id INTEGER, invited_id INTEGER, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)"
            )
            self.conn.execute("INSERT INTO referrals (inviter_id,invited_id) VALUES (?,?)", (inviter_id, invited_id))
            self.conn.commit()
        except: pass

    def get_referral_count(self, user_id):
        try:
            self.conn.execute("CREATE TABLE IF NOT EXISTS referrals (id INTEGER PRIMARY KEY AUTOINCREMENT, inviter_id INTEGER, invited_id INTEGER, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)")
            self.conn.commit()
            row = self.conn.execute("SELECT COUNT(*) FROM referrals WHERE inviter_id=?", (user_id,)).fetchone()
            return row[0] if row else 0
        except: return 0

    def referral_exists(self, invited_id):
        try:
            row = self.conn.execute("SELECT id FROM referrals WHERE invited_id=?", (invited_id,)).fetchone()
            return row is not None
        except: return False

    # ── NISHONLAR (BADGES) ────────────────────────────────────────────────────
    def get_user_test_count(self, user_id):
        """Foydalanuvchi yechgan testlar soni"""
        row = self.conn.execute(
            "SELECT COUNT(DISTINCT test_id) FROM pdf_results WHERE user_id=?", (user_id,)
        ).fetchone()
        return row[0] if row else 0

    def get_user_badges(self, user_id):
        """Foydalanuvchi nishonlarini hisoblash"""
